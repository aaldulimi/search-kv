...
logging.basicConfig(filename='log/download.log',level=logging.INFO,
...
with open("data/data2.xml", 'w') as file:
...
logging.info(f'365 day urls fetched successfully')
    ...
    try:
        async with session.get(url) as resp:
            ...
            if response_text:            
                ...
                global to_load, article_count, article_success
                to_load.append(doc)
                
                if len(to_load) >= 10:
                    with open("data/data2.xml", 'a') as file:
                        for doc in to_load:
                            file.write(doc)
                ...
                logging.info(f'{article_success}/{article_count} Downloaded successfully {title}')
        ...
        article_count += 1
        logging.warning(f'{article_count - article_success} Download failed')
        return 
